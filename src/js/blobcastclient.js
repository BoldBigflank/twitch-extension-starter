export default function BlobCastClient(cfg) {
    if (!cfg) {
        cfg = {};
    }
    this.myUserId = this.myUuid = cfg.userId;
    this.server = cfg.hasOwnProperty('server') ? cfg.server : 'https://blobcast.jackboxgames.com';
...
BlobCastClient.prototype.connectToService = function(roomId, successCallback, failCallback) {
    ...
    .then((data) => {
        ...
        if (data.hasOwnProperty('joinAs')) {
            if (data.joinAs === 'full') {
                return failCallback(new Error('The room is full'));
            }
        }
        var serverName = data.server;
        _this.socket = this.io.connect(serverName + ":38203", {
...
BlobCastClient.prototype.joinRoom = function(roomId, name, options) {
    // Room code required
    if(!roomId || roomId.length === 0) {
        this.delegate.onJoinRoomFail(new Error('Invalid Room Code'));
        return;
    }

    // Name
    // if(this.delegate.model.attributes.customOptions) {
    //     var userInfo = this.delegate.model.attributes.customOptions.userInfoTypes;
    //     if(userInfo.indexOf("email") !== -1) {
    //         if(options.email.length === 0) {
    //             this.delegate.onJoinRoomFail(new Error('Invalid Email'));
    //             return;
    //         }
    //         var validEmail = /^[\w-\.]+@([\w-]+\.)+[\w-]{1,4}$/;
    //         if (!validEmail.test(options.email)) {
    //             this.delegate.onJoinRoomFail(new Error('Invalid Email'));
    //             return;
    //         }
    //     }

    //     if(userInfo.indexOf("username") !== -1) {
    //         if(options.name.length === 0) {
    //             this.delegate.onJoinRoomFail(new Error('Invalid Name'));
    //             return;
    //         }
    //     }
    // } else {
    ...
    this.connectToService(roomId, function( appId, joinType ) {
        this.send(this.createActionPacket("JoinRoom", { roomId : roomId, name : name, appId: appId, joinType: joinType, options: options } ));
    }.bind(this),
    function(err) {
        this.delegate.onJoinRoomFail(err);
    }.bind(this));
};
...
BlobCastClient.prototype.stopsession = function(module, name) {
    ...
    this.send(this.createActionPacket("StopSession", { userId : this.myUserId, roomId : this.currentRoomId, module: module, name : name, options: options }) );
};
...
BlobCastClient.prototype.getSessionStatus = function(module, name) {
    ...
    this.send(this.createActionPacket("GetSessionStatus", { userId : this.myUserId, roomId : this.currentRoomId, module: module, name : name, options: options }) );
};
...
BlobCastClient.prototype.onDataReceived = function(data) {
    if(data.type == "Result") {
        if(data.action == "CreateRoom") {
            ...
            if(data.success) {
                ...
                if ( !this.currentRoomId ) {
                    ...
                    if(this.delegate && this.delegate.onRoomJoined) {
                        this.delegate.onRoomJoined(this.currentRoomId, data.generatedName);
                    }
                } else {
                    this.currentRoomId = data.roomId;
            ...
            this.delegate.onSendSessionMessageResult( data.success, data.module, data.name, data.response );
        }
    }
...
// window.Blobcast = BlobCastClient;